        ...
        if (args.length != 1) {
            System.out.println("Required argument: <port>");
        		 ...
        		 try {
        			 ...
        			 ShowsHandler show=new ShowsHandler();
        			 List<Show> allShows=ShowsHandler.loadShows(session);
        			 //MoviesController movie=new MoviesController();
        			 
        			 for(Show show1:allShows) {     /*one to many movies-shows*/
        				 List<Movie>AllMovies=MoviesController.loadMovies(session);
        				 index=rnd.nextInt(AllMovies.size());
        				 Movie movie =AllMovies.get(index);
        				 show1.setMovie(movie);
        				 movie.getShows().add(show1);
        			 }
        			
        			 for(Show show1:allShows) {      /*Hall-Show*/
        				 List<Hall>AllHalls=HallController.loadHalls(session);
        				 List<Show> AllShows=ShowsHandler.loadShows(session);
        			 ...
        			 for(Cinema cinema1:allCinemas) /*each cinema has one cinema manager*/
        			 {
        				 ...
        				 cinema1.setmanager(manager);
        				 manager.setCinema(cinema1);
        				 
        			 }
        			 TicketsController ticket=new TicketsController();
        			 List<Ticket>allTickets=TicketsController.loadTickets(session);
        			 
        			 for(Ticket ticket1:allTickets) {     /*Shows-tickets*/
        				 List<Show>Allshow=ShowsHandler.loadShows(session);
        				 index=rnd.nextInt(Allshow.size());
        				 Show show1 =Allshow.get(index);
        				 ticket1.setShow(show1);
        				 show1.getTickets().add(ticket1);
        			 }
        			 
        			 SeatController seat=new SeatController();
        			 List<Seat>allSeats=SeatController.loadSeats(session);
        			 
        			 for(Seat seat1:allSeats) {     /*Seats-Halls*/
        				 List<Hall>AllHall=HallController.loadHalls(session);
        				 index=rnd.nextInt(AllHall.size());
        				 Hall hall =AllHall.get(index);
        				 seat1.setHall(hall);
        				 hall.getSeats().add(seat1);
        			 }
        			 
        			 LinksController link=new LinksController();
        			 List<Link>allLinks=LinksController.loadLinks(session);
        			 
        			 for(Link link1:allLinks) {     /*Links-Shows*/
        				 List<Show>AllShow=ShowsHandler.loadShows(session);
        				 index=rnd.nextInt(AllShow.size());
        				 Show show1 =AllShow.get(index);
        				 link1.setLink(link1);
        				 show1.getLinks().add(link1);/*add link*/
        			 }
        			 
        			 for(Show show1:allShows) {     /*cinema-shows*/
        				 List<Cinema>allCinemas1=allCinemas;
        				 index=rnd.nextInt(allCinemas1.size());
        				 Cinema cinema1 =allCinemas1.get(index);
        				 show1.setCinema(cinema1);
        				 cinema1.getShows().add(show1);
        			 }
        			 for(Cinema cinema1:allCinemas) /*Cinema-Regulation*/
        			 {
        				 List<Regulations>AllReg=RegulationsController.loadReagulations(session);/*??*/
        				 List<Cinema>cinemas=allCinemas;
        				 index = rnd.nextInt(AllReg.size());        	 
        				 Regulations reg= AllReg.get(index);
        				 cinema1.setRegulation(reg);
        				 reg.setCinema(cinema1);
        				 
        			 }
        			 ComplaintsController complaint=new ComplaintsController();
        			 List<Complaint>allComplaints=ComplaintsController.loadComplaints(session);
        			 
        			 for(Complaint complaint1:allComplaints) {     /*oCustomer-Complaint????*/
